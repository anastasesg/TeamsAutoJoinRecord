...
mode: int = 3
uuid_regex: str = r"\b[0-9a-f]{8}\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-\b[0-9a-f]{12}\b"
...
def waitUntilFound(sel, timeout, print_error=True):
    try:
...
def switchToCalendarTab():
    calendar_button = waitUntilFound("button.app-bar-link > ng-include > svg.icons-calendar", 5)
    if calendar_button is not None:
        calendar_button.click()


def changeOrganisation(org_num):
    select_change_org = waitUntilFound("button.tenant-switcher", 20)
    ...
    change_org = waitUntilFound(f"li.tenant-option[aria-posinset='{org_num}']", 20)
    ...
    use_web_instead = waitUntilFound(".use-app-lnk", 5, print_error=False)
...
def preparePage(include_calendar):
    try:
    ...
    if include_calendar:
        ...
        view_switcher = waitUntilFound(".ms-CommandBar-secondaryCommand > div > button[class*='__topBarContent']", 5)
    ...
    for team in teams:
        for channel in team.channels:
            if channel.has_meeting and not channel.blacklisted:
                ...
                meeting_elem = waitUntilFound(".ts-calling-thread-header", 10)
...
def getCalendarMeetings():
    global meetings

    if waitUntilFound("div[class*='__cardHolder']", 20, False) is None:
...
def joinMeeting(meeting):
    ...
    if meeting.calendar_meeting:
        switchToCalendarTab()
        join_btn = waitUntilFound(f"div[id='{meeting.m_id}'] > div > button", 5)
    else:
        browser.execute_script(f'window.location = "{conversation_link}a?threadId={meeting.channel_id}&ctx=channel";')
        switchToTeamsTab()

        join_btn = waitUntilFound(f"div[id='{meeting.m_id}'] > calling-join-button > button", 5)
    ...
    join_now_btn = waitUntilFound("button[data-tid='prejoin-join-button']", 30)
    ...
    join_now_btn = waitUntilFound("button[data-tid='prejoin-join-button']", 5)
    if join_now_btn is None:
        return
    join_now_btn.click()
    ...
    if "join_message" in config and config["join_message"] != "":
        ...
        try:
            ...
            text_input = waitUntilFound('div[role="textbox"] > div', 5)
            ...
            time.sleep(3)
            send_button = waitUntilFound("#send-message-button", 5)
            send_button.click()
...
def handleLeaveLogic(current_members, total_members):
    ...
    if leave_number != "":
        ...
        if 0 < float(leave_percentage) <= 150:
            ...
            print(leave_percentage + "is not a valid value for threshold. Threshold percent must be greater than 0 "
                                     "and less than 100.")
            return False
...
def main():
    ...
    if config['email'] != "" and config['password'] != "":
        ...
        login_user = waitUntilFound("input[type='text']", 5)
        if login_user is not None:
            login_user.send_keys(config['email'][:8])

        login_pwd = waitUntilFound("input[type='password']", 10)
        if login_pwd is not None:
            login_pwd.send_keys(config['password'])

        # find the element again to avoid StaleElementReferenceException
        login_pwd = waitUntilFound("input[type='password']", 5)
        if login_pwd is not None:
    ...
    if 'organisation_num' in config and config['organisation_num'] > 1:
        changeOrganisation(config['organisation_num'])
    ...
    if mode != 2:
        ...
        preparePage(include_calendar=False)
    ...
    while 1:
        ...
        if "pause_search" in config and config['pause_search'] and current_meeting is not None:
            ...
            if len(meetings) > 0:
                ...
                meeting_to_join = decideMeeting()
                if meeting_to_join is not None:
                    total_members = 0
                    joinMeeting(meeting_to_join)
                    startRecording()
